...
export default async function JamSharedSongs({ jamId }: { jamId: string }) {
  const supabase = await supabaseServer();
  const { data } = await supabase.rpc("jam_shared_songs", { jam_id_param: jamId });
  const songs = (data as SharedSong[] | null) ?? [];
