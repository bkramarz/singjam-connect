-- Migration 034: fix composer/culture/production aggregation being filtered by WHERE clause.
-- Previously, searching "john lennon" would drop McCartney rows before aggregation.
-- Solution: find matching song IDs first (CTE), then re-join all tables for those songs.
...
as $$
  ...
  -- Step 1: identify matching song IDs only (WHERE filtering happens here)
  matched as (
  ...
  -- Step 2: re-join all tables for matched songs with no WHERE filter, so aggregates are complete
  select
    s.id             as song_id,
    s.title,
    s.display_artist,
