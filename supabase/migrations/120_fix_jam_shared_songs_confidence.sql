-- Migration 120: fix 'follow' → 'learn' in jam_shared_songs
-- Same enum mismatch fixed in migration 119 for set_shared_songs/solo_set_songs.
-- Migration 051 removed 'follow' from the confidence enum; this function was never updated.
...
as $$
  ...
  linked_set_songs as (
    select ss.song_id
    from public.sets st
    join public.set_songs ss on ss.set_id = st.id
    where st.jam_id = jam_id_param
  )
  select
    combined.song_id,
    combined.slug,
    ...
    exists (select 1 from linked_set_songs where song_id = combined.song_id) as in_set
  from (
